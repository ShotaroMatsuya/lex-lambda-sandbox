...
def generate_car_price(location, days, age, car_type):
    ...
    return days * (
        (100 + base_location_cost)
        + ((car_types.index(car_type.lower()) * 50) * age_multiplier)
    )
...
def validate_book_car(slots):
    ...
    if pickup_date:
        ...
        if (
            datetime.datetime.strptime(pickup_date, "%Y-%m-%d").date()
            <= datetime.date.today()
        ):
            return build_validation_result(
                False,
...
def book_hotel(intent_request):
    ...
    checkin_date = try_ex(
        lambda: intent_request["currentIntent"]["slots"]["CheckInDate"]
    )
    nights = safe_int(
        try_ex(lambda: intent_request["currentIntent"]["slots"]["Nights"])
    )
...
def book_car(intent_request):
    ...
    last_confirmed_reservation = try_ex(
        lambda: session_attributes["lastConfirmedReservation"]
    )
    ...
    if pickup_city and pickup_date and return_date and driver_age and car_type:
        ...
        price = generate_car_price(
            pickup_city,
            get_day_difference(pickup_date, return_date),
            driver_age,
            car_type,
        )
        session_attributes["currentReservationPrice"] = price
    ...
    if intent_request["invocationSource"] == "DialogCodeHook":
        ...
        if confirmation_status == "None":
            ...
            if (
                ...
                if (
                    last_confirmed_reservation
                    and try_ex(lambda: last_confirmed_reservation["ReservationType"])
                    == "Hotel"
                ):
